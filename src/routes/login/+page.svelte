    ...
    const handleSignup = async () => {
        ...
        try {
           ...
           await updateProfile(auth.currentUser, {
            displayName: name, photoURL: "https://img.freepik.com/free-vector/blue-circle-with-white-user_78370-4707.jpg"
            }).then(() => {
...
{/if}
                ...
                {#if signUp}
                    ...
                    <form class="flex flex-col gap-2 w-full justify-center" on:submit|preventDefault={handleSignup}>
                        ...
                        <div class="flex items-start pt-1 pb-1">
                            <input type="checkbox" id="terms" bind:checked={termsAccepted} required /> <!-- Added checkbox with binding -->
                        ...
                        <button type="submit" class="w-full rounded-lg bg-[#294254] p-2 text-white text-center" disabled={!termsAccepted}>Signup with Email</button> <!-- Disable button if terms not accepted -->
                    </form>
                {:else}
                ...
                </button> <!-- Disable button if terms not accepted -->
                <div class="link">
                    <p class="mb-2 -mt-2 font-medium">Already have an account? <a href="/login" class="text-blue-600" on:click={()=>{signUp=!signUp}}>Log In</a></p>
                </div>
            </Modal>
        {/if}
    </div>
